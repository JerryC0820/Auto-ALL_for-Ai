...
MANIFEST_URL_GITEE = "https://gitee.com/chen-bin98/Auto-ALL_for-Ai/raw/main/latest/manifest.json"
MANIFEST_URL_GITHUB = "https://raw.githubusercontent.com/JerryC0820/Auto-ALL_for-Ai/main/latest/manifest.json"
...
def load_settings():
    ...
    file_defaults = _load_default_settings_file()
    if isinstance(file_defaults, dict) and file_defaults:
        default.update(file_defaults)
    raw = {}
    try:
...
def _iter_manifest_sources(prefer_source: str):
    if prefer_source == UPDATE_SOURCE_GITHUB:
        return [(UPDATE_SOURCE_GITHUB, MANIFEST_URL_GITHUB), (UPDATE_SOURCE_GITEE, MANIFEST_URL_GITEE)]
    if prefer_source == UPDATE_SOURCE_GITEE:
        return [(UPDATE_SOURCE_GITEE, MANIFEST_URL_GITEE), (UPDATE_SOURCE_GITHUB, MANIFEST_URL_GITHUB)]
    return [(UPDATE_SOURCE_GITEE, MANIFEST_URL_GITEE), (UPDATE_SOURCE_GITHUB, MANIFEST_URL_GITHUB)]

def _parse_manifest_ts(manifest: dict) -> float:
    if not manifest:
        return 0.0
    ts = manifest.get("release_ts")
    if ts:
        try:
            return float(ts)
        except Exception:
            pass
    for key in ("release_time", "published_at", "updated_at", "created_at"):
        value = (manifest.get(key) or "").strip()
        if value:
            return _parse_release_ts(value)
    return 0.0

def _manifest_to_update_info(manifest: dict, source_key: str, current_version: str, last_release_ts: float):
    if not isinstance(manifest, dict):
        return None
    version = (manifest.get("version") or manifest.get("tag") or "").strip()
    release_ts = _parse_manifest_ts(manifest)
    cur_ver = _parse_version(current_version)
    new_ver = _parse_version(version) if version else None
    same_version_update = False
    if new_ver and cur_ver:
        if _version_gt(new_ver, cur_ver):
            pass
        elif new_ver == cur_ver and release_ts and release_ts > float(last_release_ts or 0):
            same_version_update = True
        else:
            return None
    elif release_ts and release_ts > float(last_release_ts or 0):
        same_version_update = True
    else:
        return None
    ...
    package_url = (
        pkg.get(f"url_{source_key}")
        or manifest.get(f"package_url_{source_key}")
        or pkg.get("url")
        or manifest.get("package_url")
        or ""
    ).strip()
    package_name = (pkg.get("name") or os.path.basename(package_url) or "").strip()
    package_size = int(pkg.get("size") or manifest.get("package_size") or 0)
    if not package_url:
        return None
    info = {
        "source": source_key,
        ...
        "tag": version or "",
        "name": manifest.get("title") or package_name or version or "更新包",
        "body": manifest.get("notes") or "",
        "published_at": manifest.get("release_time") or manifest.get("published_at") or "",
        "release_ts": release_ts,
        "same_version_update": same_version_update,
        "asset_name": package_name or "update_package.zip",
        "asset_url": package_url,
        "asset_size": package_size,
        "notes_url": (
            manifest.get(f"notes_url_{source_key}")
            or manifest.get("notes_url")
            or ""
        ),
        "history": manifest.get("history") or [],
        "force": bool(manifest.get("force") or False),
        "rules_url": (
            manifest.get(f"rules_url_{source_key}")
            or manifest.get("rules_url")
            or ""
        ),
        "installer_url": (
            (manifest.get("installer") or {}).get(f"url_{source_key}")
            or (manifest.get("installer") or {}).get("url")
            or manifest.get(f"installer_url_{source_key}")
...
def check_manifest_update(current_version: str, last_release_ts: float, prefer_source: str):
    token = _get_env_token("GITEE_TOKEN", "GITEE_ACCESS_TOKEN")
    for source_key, url in _iter_manifest_sources(prefer_source):
        try:
            manifest_url = url
            if source_key == UPDATE_SOURCE_GITEE and token:
                manifest_url = _append_query_param(manifest_url, "access_token", token)
            data = _http_get_json(manifest_url, timeout=UPDATE_CHECK_TIMEOUT)
            info = _manifest_to_update_info(data, source_key, current_version, last_release_ts)
            if info:
                return info, ""
        except Exception:
            continue
...
def check_update_by_source(current_version: str, update_source: str, last_release_ts: float = 0):
    source = update_source or DEFAULT_UPDATE_SOURCE
    info, err = check_manifest_update(current_version, last_release_ts, source)
    if info:
        return info, ""
    if source == UPDATE_SOURCE_GITEE:
...
def download_update_package(info: dict, progress_cb=None):
    try:
        ...
        rules_url = info.get("rules_url") or ""
        if info.get("source") == UPDATE_SOURCE_GITEE:
            token = _get_env_token("GITEE_TOKEN", "GITEE_ACCESS_TOKEN")
            url = _append_query_param(url, "access_token", token)
            if rules_url:
                rules_url = _append_query_param(rules_url, "access_token", token)
        if not url:
            return None, "更新包链接无效"
        ...
        if rules_url:
            try:
                rules_path = os.path.join(base_dir, "_mini_fish_update_rules.json")
                _download_file(rules_url, rules_path, expected_size=0)
            except Exception:
                rules_path = ""
        result = {
...
class MiniFish(QtWidgets.QWidget):
    ...
    def _show_update_dialog(self, info: dict, required: bool) -> bool:
        ...
        notes_url = (info.get("notes_url") or "").strip()
        if notes_url:
            notes_html += f"<div style=\"margin-top:8px;\"><a href=\"{notes_url}\">查看完整更新说明</a></div>"
    ...
    def _on_update_checked(self, info, err: str):
        ...
        if info.get("force"):
            ...
            self._prompt_update_required(info)
            return
        if force:
            if new_version:
                self._update_prompted_version = new_version
    ...
    def _run_update_script(self, result: dict):
        try:
            ...
            ps_path = os.path.join(tempfile.gettempdir(), f"_mini_fish_update_{ts}.ps1")
            rules_path = result.get("rules_path") or ""
            vbs_script = "\r\n".join([
            ...
            ps_script = "\r\n".join([
                "$rulesPath = $env:RULES",
                "if (-not $rulesPath) { exit 0 }",
                "if (-not (Test-Path -LiteralPath $rulesPath)) { exit 0 }",
                "$base = $env:DST",
                "try { $rules = Get-Content -LiteralPath $rulesPath -Raw | ConvertFrom-Json } catch { exit 0 }",
                "function Join-Target([string]$p){ Join-Path -Path $base -ChildPath $p }",
                "if ($rules.delete) {",
                "  foreach ($p in $rules.delete) {",
                "    $t = Join-Target $p",
                "    if (Test-Path -LiteralPath $t) { Remove-Item -LiteralPath $t -Recurse -Force -ErrorAction SilentlyContinue }",
                "  }",
                "}",
                "if ($rules.move) {",
                "  foreach ($m in $rules.move) {",
                "    if (-not $m.from) { continue }",
                "    $src = Join-Target $m.from",
                "    $dst = Join-Target $m.to",
                ...
                "}",
                "if ($rules.copy) {",
                "  foreach ($c in $rules.copy) {",
                "    if (-not $c.from) { continue }",
                "    $src = Join-Target $c.from",
                "    $dst = Join-Target $c.to",
                "    if (Test-Path -LiteralPath $src) {",
                "      $dir = Split-Path -Path $dst -Parent",
                "      if ($dir) { New-Item -ItemType Directory -Path $dir -Force | Out-Null }",
            ...
            cmd_lines = [
                ...
                "if exist \"%SRC%\" robocopy \"%SRC%\" \"%DST%\" /E /R:1 /W:1 /XD \"_internal\" \"_mini_fish_cache\" \"_mini_fish_icons\" \"_mini_fish_profile\" /XF \"_mini_fish_settings.json\" \"_mini_fish_settings_bootstrap.json\" >nul",
                "if /I not \"%OLDEXE%\"==\"%DST%\\%NEWEXE%\" if exist \"%OLDEXE%\" del /f /q \"%OLDEXE%\"",
                "if exist \"%RULES%\" powershell -NoProfile -ExecutionPolicy Bypass -File \"%PS1%\"",
