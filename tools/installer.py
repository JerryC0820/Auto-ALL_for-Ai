...
def fetch_manifest(prefer_source: str = "gitee"):
    token = os.environ.get("GITEE_TOKEN") or os.environ.get("GITEE_ACCESS_TOKEN") or ""
    for url in _iter_preferred_urls(prefer_source, MANIFEST_URL_GITEE, MANIFEST_URL_GITHUB):
        try:
            manifest_url = url
            if "gitee.com" in manifest_url and token:
                sep = "&" if "?" in manifest_url else "?"
                manifest_url = f"{manifest_url}{sep}access_token={token}"
            data = _fetch_json(manifest_url)
            pkg = data.get("package") or {}
            source_key = "gitee" if "gitee.com" in url else "github"
            package_url = (
                pkg.get(f"url_{source_key}")
                or data.get(f"package_url_{source_key}")
                or pkg.get("url")
            ...
            if not package_url:
                continue
            if "gitee.com" in package_url and token:
                sep = "&" if "?" in package_url else "?"
                package_url = f"{package_url}{sep}access_token={token}"
            return {
                "source": source_key,
                "version": data.get("version") or "",
                "name": pkg.get("name") or os.path.basename(package_url) or "",
                "url": package_url,
                "size": int(pkg.get("size") or data.get("package_size") or 0),
            }
        except Exception:
            continue
    return None
...
def fetch_latest_release(prefer_source: str = "gitee"):
    manifest = fetch_manifest(prefer_source)
    if manifest:
        return manifest
...
class InstallerApp(tk.Tk):
    ...
    def _init_fonts(self):
        try:
            ...
            try:
                ...
                pass
            style.configure("TButton", font=(FONT_FAMILY, FONT_BASE_SIZE), padding=(14, 6))
            style.configure("Footer.TButton", font=(FONT_FAMILY, FONT_BASE_SIZE), padding=(14, 6))
            style.map("Footer.TButton", foreground=[("disabled", "#9E9E9E")])
            style.configure("TCheckbutton", font=(FONT_FAMILY, FONT_BASE_SIZE))
            style.configure("TRadiobutton", font=(FONT_FAMILY, FONT_BASE_SIZE))
            style.configure("TLabel", font=(FONT_FAMILY, FONT_BASE_SIZE), foreground="#222222")
    ...
    def _estimate_required_mb(self):
        if self.release_info and self.release_info.get("size"):
            ...
            return max(size_mb, DEFAULT_REQUIRED_MB), ""
    ...
    def _create_page(self, title_key: str, subtitle_key: str):
        ...
        if self.brand_icon is not None:
            ...
            icon_label.grid(row=0, column=1, rowspan=2, sticky="ne", padx=(0, 4))

        header.columnconfigure(0, weight=1)

        body_outer = tk.Frame(frame, bg=COLOR_PAGE_BG)
        body_outer.pack(fill="both", expand=True, padx=28, pady=(0, 12))
        content = tk.Frame(body_outer, bg=COLOR_PANEL_BG)
    ...
    def _build_step1(self):
        ...
        text.configure(spacing1=4, spacing3=4, font=(FONT_FAMILY, FONT_BASE_SIZE))
    ...
    def _build_step4(self):
        ...
        group = ttk.LabelFrame(body, text=TEXT["task_group_download"], padding=(10, 6))
        group.pack(fill="x", pady=(16, 4))
        desc = tk.Label(
            group,
            text=TEXT["task_group_download_hint"],
            font=(FONT_FAMILY, FONT_BASE_SIZE),
            fg=COLOR_TEXT_HINT,
            bg=COLOR_PANEL_BG,
        )
    ...
    def _build_step6(self):
        ...
        status = tk.Label(body, textvariable=self.status_var, font=(FONT_FAMILY, FONT_BASE_SIZE, "bold"), bg=COLOR_PANEL_BG)
    ...
    def _build_step7(self):
        ...
        finish_row.pack(fill="both", expand=True, padx=12, pady=12)
        if self.brand_icon is not None:
            icon_label = tk.Label(finish_row, image=self.brand_icon, bg=COLOR_PANEL_BG)
            icon_label.grid(row=0, column=0, rowspan=3, sticky="nw", padx=(0, 22), pady=(4, 0))

        title = tk.Label(
            finish_row,
            text=TEXT["subtitle_finish"].format(app=APP_NAME),
            font=(FONT_FAMILY, FONT_TITLE_SIZE, "bold"),
            bg=COLOR_PANEL_BG,
        )
        title.grid(row=0, column=1, sticky="w", pady=(4, 8))
        desc = tk.Label(
            finish_row,
    ...
    def _build_footer(self):
        ...
        footer = tk.Frame(self.container, bg=COLOR_FOOTER_BG)
        footer.grid(row=2, column=0, sticky="ew", padx=28, pady=(10, 12))
        footer.columnconfigure(0, weight=1)

        self.btn_back = ttk.Button(footer, text=TEXT["btn_back"], command=self._go_back, style="Footer.TButton")
        self.btn_next = ttk.Button(footer, text=TEXT["btn_next"], command=self._go_next, style="Footer.TButton")
        self.btn_cancel = ttk.Button(footer, text=TEXT["btn_cancel"], command=self._on_cancel, style="Footer.TButton")
        self.btn_finish = ttk.Button(footer, text=TEXT["btn_finish"], command=self._finish, style="Footer.TButton")
        ...
        try:
            while True:
                ...
                if not msg:
                    continue
                if msg[0] == "step":
                    self.step_var.set(TEXT["step_label"].format(step=msg[1]))
                elif msg[0] == "detail":
