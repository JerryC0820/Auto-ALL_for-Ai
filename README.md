...
## 📝 更新内容
- 新增自动更新（国内优先 + 下载进度），可自动下载并重启
- 更新源可在“关于”内切换（Gitee/GitHub）
- 关于按钮红点提示更直观（发现新版本即显示）
- 缺少配置时自动拉取当前版本默认配置；若有新版本则强制更新
...
## 💻 单命令安装/启动（终端）
```
py -3 -m pip install -r requirements.txt && py 牛马神器_v2.0.46.py
```
（在仓库目录内执行）
...
## 📦 EXE 版（已打包）
- 位置：`dist\牛马神器_v2.0.46\牛马神器_v2.0.46.exe`
- 直接双击运行即可
...
在仓库目录执行：
```
py -m PyInstaller --noconfirm --clean --name "牛马神器_v2.0.46" --icon "assets\牛马爱摸鱼V2.01.ico" --add-data "assets;assets" --windowed "牛马神器_v2.0.46.py"
```
输出目录：`dist\牛马神器_v2.0.46\`
