...
## 📦 EXE 版（已打包）
- 标准版（需 `_internal`）：`dist\牛马神器_v4.0.10\牛马神器_v4.0.10.exe`
- 单文件版（无需 `_internal`）：`dist\牛马神器_v4.0.10_onefile.exe`
- 直接双击运行即可
...
在仓库目录执行：
```
py -m PyInstaller --noconfirm --clean --name "牛马神器_v4.0.10" --icon "assets\牛马爱摸鱼V2.01.ico" --add-data "assets;assets" --windowed "牛马神器_v4.0.10.py"
```
输出目录：`dist\牛马神器_v4.0.10\`
