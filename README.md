...
## 💻 单命令安装/启动（终端）
```
py -3 -m pip install -r requirements.txt && py 牛马神器_v4.0.11.py
```
（在仓库目录内执行）
...
## 📦 EXE 版（已打包）
- 标准版（需 `_internal`）：`dist\牛马神器_v4.0.11\牛马神器_v4.0.11.exe`
- 单文件版（无需 `_internal`）：`dist\牛马神器_v4.0.11_onefile.exe`
- 直接双击运行即可
...
在仓库目录执行：
```
py -m PyInstaller --noconfirm --clean --name "牛马神器_v4.0.11" --icon "assets\牛马爱摸鱼V2.01.ico" --add-data "assets;assets" --windowed "牛马神器_v4.0.11.py"
```
输出目录：`dist\牛马神器_v4.0.11\`
